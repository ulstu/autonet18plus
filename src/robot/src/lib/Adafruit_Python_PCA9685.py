...
class PCA9685:
    ...
    def setServo(self, channel, dutycycle):
        ...
        if self.servos[channel].signed == True:
            value = SERVO_CENTER + self.servos[channel].servo_trim + (
                        self.servos[channel].servo_max - self.servos[channel].servo_min) * exp_val / 2.0
        else:
            value = self.servos[channel].servo_min + self.servos[channel].servo_trim + (
                        self.servos[channel].servo_max - self.servos[channel].servo_min) * exp_val
